...
const ChangePassword = ({ setLinear }) => {
    ...
    if (userInfo?.info && !userInfo?.info?.message && about.length === 0 && keys.length === 0) {
        const temp = Object.keys(userInfo?.info);
                                        ...
                                        <Typography className={classes.myAlert}>
                                            {
                                                keys.map((key, index) => {
                                                    ...
                                                    return (
                                                        <>
                                                            <Alert key={`${index}password`} severity="success" style={{ borderRadius: '20px', display: 'flex', overflow: 'auto' }}>
                                        ...
                                        loading ? (<CircularProgress size={68} className={classes.fabProgress} />) : (
                                            <>
                                                <form autoComplete="off" onSubmit={handleSave}>
                                                    ...
                                                    <InputEdit required name="oldPassword" type="password" label="Old Password" handleChange={handleChange} icon={<LockIcon />} />
                                                    <InputEdit required name="newPassword" type="password" label="New Password" handleChange={handleChange} icon={<LockOpenIcon />} />
